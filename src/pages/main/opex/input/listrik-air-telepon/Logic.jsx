...
const Logic = () => {
  ...
  const onChangeTable = async (change, i, category) => {
    ...
    if (type === "text") {
      ...
      let totalPemakaian = newRows[i][rowIndex].cells[3].value;
      let tarif = newRows[i][rowIndex].cells[4].value;

      let totalBiaya = totalPemakaian * tarif;

      let grandTotal = 0;

      const newCell = newRows[i][rowIndex].cells.map((e, j) => {
        if (j === 5) e.value = totalBiaya;
        if (j >= 8) {
          e.value = totalBiaya;
          grandTotal += e.value;
        }
    ...
    try {
      ...
      const newCell = newRows[i][rowIndex].cells.map((e, i) => {
        if (i >= 1 && i <= 4) e.nonEditable = false;
        if (i === 6) e.nonEditable = false;
        return e;
      });
