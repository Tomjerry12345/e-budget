...
const PenjualanHkLogic = () => {
  ...
  const { response, errorMessage, nameReducer } = useSelector((state) => state.reducer);
  ...
  const onSetDataTable = async (values) => {
    ...
    const resListAsumsi = await getData(fCodeCompany, fCodeLocation, fCodeDept, childUrl[0].endPoint, childUrl[0].name, childUrl[0].update);
    const resListHarga = await getData(fCodeCompany, fCodeLocation, fCodeDept, childUrl[1].endPoint, childUrl[1].name, childUrl[1].update);
    const resListPenjualan = await getData(fCodeCompany, fCodeLocation, fCodeDept, childUrl[2].endPoint, childUrl[2].name, childUrl[2].update);
    const resListPotongan = await getData(fCodeCompany, fCodeLocation, fCodeDept, childUrl[3].endPoint, childUrl[3].name, childUrl[3].update);

    setDataColumnInput({ ...dataColumnInput, [childUrl[0].name]: resListAsumsi.list, [childUrl[1].name]: resListHarga.list, [childUrl[2].name]: resListPenjualan.list, [childUrl[3].name]: resListPotongan.list });
  ...
  const getData = async (codeCompany, codeLocation, codeDept, endPoint, name, update) => {
    ...
    const { list, column } = getDataTable(data, name, update);
  ...
  const getDataTable = (response, name, e) => {
    ...
    const column = onSetColumn(year_1, year_2, keyParent, name);
...
export default PenjualanHkLogic;
