...
import { getRootHeaderRow, getRows, updateTotalRow } from "./getRows";
...
const Logic = () => {
  ...
  const ENDPOINT_URL = "detail-mpp/mpp-others";
    ...
    try {
      for (const c of change) {
        ...
        if (type === "text") {
          ...
          if (!isNaN(value)) {
            ...
            const newCell = newRows[rowIndex].cells.map((e, j) => {
              if (j >= 2 && j <= 5) total1 += e.value;
              if (j === 6) e.value = total1;
              if (j >= 7 && j <= 18) total2 += e.value;
              if (j === 19) e.value = total2;
              return e;
            });
        ...
        if (isChange) {
          ...
          const codeAccount = newRows[rowIndex].codeAccount;
